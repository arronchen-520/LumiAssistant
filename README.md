...
核心亮点：**仅需调用一次大模型**，就能准确理解你说的话，并在 7 种不同的意图中做出响应。短暂的闲聊片段不会被保存进日记数据库，保持日记本的纯洁性。
